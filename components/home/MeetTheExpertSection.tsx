...
import { Dna, BrainCircuit, Sparkles, Users } from 'lucide-react';
  ...
  const keyPrinciples = [
    {
      ...
      title: "Bio (The Body)",
      description: "Addressing health with a full scientific arsenal including pharmacology, biochemistry and more."
    },
    {
      icon: <BrainCircuit className="h-8 w-8 text-brand-600 dark:text-brand-400" />,
      title: "Psycho (The Mind)",
      description: "Recognizing the connection between thoughts, emotions, and physical health."
    },
    {
      icon: <Users className="h-8 w-8 text-brand-600 dark:text-brand-400" />,
      title: "Social (The Environment)",
      description: "Understanding the impact of family, work, and community on well-being."
    },
    {
      icon: <Sparkles className="h-8 w-8 text-brand-600 dark:text-brand-400" />,
      title: "Spiritual (The Purpose)",
      description: "Addressing the need for meaning and hope as a critical component of healing."
    }
  ];
    ...
    <AnimatedSection id="expert">
      <div className="grid lg:grid-cols-2 gap-12 items-start">
        ...
        <div>
          ...
          <div className="mt-4 text-slate-600 dark:text-slate-300 prose prose-sm max-w-none dark:prose-invert" dangerouslySetInnerHTML={{ __html: expertBio }} />
            ...
            {keyPrinciples.map((principle, index) => (
              <div key={index} className="p-4 rounded-2xl bg-slate-100/60 dark:bg-slate-900/30 border border-slate-200/70 dark:border-slate-800 h-full">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 h-12 w-12 grid place-items-center rounded-xl bg-white dark:bg-slate-800 shadow-sm">
                    {principle.icon}
                  </div>
                  <div>
